...
class PlayerAgent:
    """
    STYLE B-PRIME SAFE+ VERSION
    --------------------------------------
    - Early game: extremely trapdoor-safe, but still blocks and pressures opponent
    - Mid game: begins exploring safely, reduces aggression
    - Late game: strong safe egg-laying, maximum avoidance of danger
    """
    ...
    def _backtrack_penalty(self, dest):
        return 7.0 if dest == self.prev_loc else 0
    ...
    # ------------------------------------------------------------
    # Immediate utility (SAFETY-FIRST early game)
    # ------------------------------------------------------------
    def _immediate_utility(self, move, board_state, sensors):
        ...
        # ======================================================
        # SAFETY FIRST: HARD TRAPDOOR BLOCK
        # ======================================================
        trap_risk = self._trapdoor_risk_at(dest_x, dest_y)

        if phase == "early" and trap_risk > 0.10:
            return -5000

        # Prevent eggs or turds on risky squares
        if "egg" in name and trap_risk > 0.08:
            return -500
        if "turd" in name and trap_risk > 0.08:
            return -300

        # ======================================================
        # 1. Egg placement (safe-first egg strategy)
        # ======================================================
        if "egg" in name:
            if phase == "early":
                util += 50
            ...
            # reward safe egg tiles
            if trap_risk < 0.05:
                util += 15

        # ======================================================
        # 2. Turds — safe blocking only
        # ======================================================
        if "turd" in name:
            ...
            if phase == "early":
                if dist <= 2 and trap_risk < 0.07:
                    util += 25
                elif dist <= 3 and trap_risk < 0.07:
                    util += 10
                else:
                    util -= 10
        ...
        if phase == "early":
            ...
            if after < before and trap_risk < 0.08:
                util += 1.2
        else:
            if after < before:
                util -= 2
        ...
        # ======================================================
        # 4. Trapdoor penalty (strong early)
        # ======================================================
        if phase == "early":
            trap_penalty = 160
        elif phase == "mid":
        ...
        # ======================================================
        # 7. Safe exploration bonus (mid/late only)
        # ======================================================
        if phase != "early" and trap_risk < 0.05:
            util += 3
    ...
    # ------------------------------------------------------------
    # Choose move with minimax weighting
    # ------------------------------------------------------------
    def _choose_move(self, board_state, sensors, time_left):
        ...
        # EGGS always immediate priority
        egg_moves = [m for m in moves if "egg" in self._enum(m[1]).lower()]
        if egg_moves:
            return max(egg_moves,
                       key=lambda m: self._immediate_utility(m, board_state, sensors))
        ...
        for _, m in scored:
            ...
            if nb is None:
                ...
                try:
                    ...
                    pass
                future = self._alpha_beta(nb, depth - 1,
                                          -1e9, 1e9, False, time_left, sensors)
