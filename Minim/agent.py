...
"""
Ulysses: a utility-maximizing chicken agent with
...
- Alpha–beta minimax search (depth 2–4) using forecast_move() and
  reverse_perspective().
...
class PlayerAgent:
    ...
    def _init_trapdoor_beliefs(self):
        ...
        for y in range(self.board_size):
            for x in range(self.board_size):
                if (x + y) % 2 == 0:  # white squares (i + j even)
        ...
        for y in range(self.board_size):
            for x in range(self.board_size):
                if (x + y) % 2 == 1:  # black squares (i + j odd)
    ...
    def _choose_move_alpha_beta(
        ...
        """
        Root-level move selection using alpha-beta search.
        ...
        for move in moves:
            ...
            # Apply our move
            try:
                next_board = board_state.forecast_move(direction, move_type)
            except (AttributeError, TypeError):
        ...
        if not best_moves:
            # Fall back if something weird happens
            return moves[0]
    ...
    def play(
        ...
        using trapdoor beliefs + alpha-beta search.
        """
        self.turn_index += 1
