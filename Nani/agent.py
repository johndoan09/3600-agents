...
"""
Minim: more egg-hungry, less scared version

- Trapdoor belief tracking (white + black).
- Sensor-based Bayesian updates using (heard, felt) probabilities.
- Trapdoor risk avoidance, but less paranoid.
- Strong priority for Egg moves.
- Mild penalty for camping on corners/edges.
...
class PlayerAgent:
    ...
    def __init__(self, board: board.Board, time_left: Callable):
        ...
        # Dialed down so we're not terrified of the board.
        self.trapdoor_penalty = 250.0
    ...
    def _corner_camping_penalty(self, x: int, y: int) -> float:
        """
        Mild penalty for being near the edges/corners so we don't camp forever.

        corner_dist = 0 at corners, 1 at edge just inside corner, etc.
        """
        d_edge = min(x, y, self.board_size - 1 - x, self.board_size - 1 - y)
        if d_edge <= 0:
            return 4.0  # strongest penalty for actual corner
        elif d_edge == 1:
            return 2.0  # still discouraged
        else:
            return 0.0
    ...
    def _immediate_utility_of_move(
        ...
        - - (scaled) trapdoor_penalty * high-risk component of P(trapdoor at dest)
        - - small corner/edge camping penalty
        """
        ...
        # Trapdoor risk penalty only for fairly high risk.
        risk = self._trapdoor_risk_at(dest_x, dest_y)
        high_risk_component = max(0.0, risk - 0.3)  # ignore tiny risk
        utility -= self.trapdoor_penalty * high_risk_component

        # Penalty for camping near corners/edges.
    ...
    def _evaluate_board(self, board_state: board.Board) -> float:
        ...
        - Mild penalty for camping on corner/edge.
        - Mild penalty for being on a very risky square.
        """
        ...
        # Corner/edge camping penalty.
        value -= self._corner_camping_penalty(x, y)

        # Penalty for standing directly on a high-risk square.
        risk_here = self._trapdoor_risk_at(x, y)
        high_risk_component = max(0.0, risk_here - 0.3)
    ...
    def _choose_move_alpha_beta(
        ...
        - If any Egg move is available, always pick an Egg move (highest immediate utility).
        - Otherwise, uses alpha-beta with iterative deepening and move ordering.
        """
        moves = board_state.get_valid_moves()
        if not moves:
            return moves[0] if moves else None

        # 1) If there are Egg moves, always choose among those (greedy egg priority).
        egg_moves = []
        for m in moves:
        ...
        if egg_moves:
            # Choose Egg move with best immediate utility.
            best_val = -1e9
            best_egg_moves: List[Tuple] = []
            for m in egg_moves:
                ...
                if val > best_val:
                    ...
                    best_egg_moves.append(m)
            return best_egg_moves[np.random.randint(len(best_egg_moves))]

        # 2) No Egg moves: run minimax / alpha–beta.
    ...
    def play(
        ...
        # Use alpha-beta search to pick a move (with greedy egg preference).
