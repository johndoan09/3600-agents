...
# ================================================================
# PlayerAgent – APEX (Dave's Board Control + Nova's Turds + 0.05 Traps)
# ================================================================
class PlayerAgent:
    """
    APEX: The ultimate agent combining:
    1. AGGRESSIVE board control (from Dave) - claim 60%+ of board
    2. DISPERSED turd placement (from Nova) - center-focused, no clustering
    3. STRICT safety (from Void) - target 0.05 traps/game
    
    Key insight: Explore FAST but SMART. Use accumulated evidence.
    Place turds strategically in center to block opponent.
    """
        ...
        for dy in range(-2, 3):
            for dx in range(-2, 3):
                ...
                if heard or felt:
                    ...
                    # APEX: Fast evidence building for aggression
    ...
    def _get_confidence(self, x, y) -> float:
        ...
        # APEX: Require 4 observations for full confidence (balance of speed and safety)
    ...
    def _score_turd_placement(self, cx, cy) -> float:
        """
        APEX: Combines Dave's confinement strategy with Nova's dispersion.
        
        1. Start earlier than Dave (turn 15 vs 20)
        2. Lower board control requirement (30% vs 40%)
        3. NO CLUSTERING (from Nova)
        4. CENTER PRIORITY (from Nova)
        5. CONFINEMENT BONUS (from Dave)
        """
        score = -50
        
        # 1. TIMING: Earlier start than Dave, but not too early
        if self.turn_index < 12:
            return -500  # Too early
        elif self.turn_index < 18:
            score -= 30  # Slight discourage
        elif self.turn_index < 28:
            score += 60  # Good window
        else:
            score += 100  # Late game - use them!
        
        # 2. BOARD CONTROL: Lower threshold than Dave (30% vs 40%)
        if self.board_control < 0.30:
            return -300  # Need some control first
        
        # Extra bonus if we have strong control
        if self.board_control >= 0.50:
            score += 80
        elif self.board_control >= 0.40:
            score += 40
        
        # 3. NO CLUSTERING (from Nova) - Critical!
        nearby_turds = self._count_nearby_turds(cx, cy, radius=2)
        if nearby_turds >= 2:
            score -= 400  # NEVER cluster
        elif nearby_turds == 1:
            score -= 180  # Avoid
        
        # 4. QUADRANT BALANCE (from Nova)
        quadrant = self._get_quadrant(cx, cy)
        turds_in_quadrant = self._count_turds_in_quadrant(quadrant)
        
        if turds_in_quadrant == 0:
            score += 140  # First in quadrant
        elif turds_in_quadrant == 1:
            score -= 40
        else:
            score -= 120
        
        # 5. CENTRAL PRIORITY (from Nova) - Block high-traffic areas
        center = self.board_size / 2 - 0.5
        ...
        if dist_to_center <= 2:
            ...
            score += 100  # Near center
        elif dist_to_center <= 4:
            score += 50   # Mid-board
        else:
            score -= 40   # Edges less valuable
        
        # 6. OPPONENT CONFINEMENT (from Dave) - Cut off their territory
        if self.enemy_loc:
            ox, oy = self.enemy_loc
            dist = abs(cx - ox) + abs(cy - oy)
            
            # Good distance for blocking path
            if dist == 2:
                score += 140
            elif dist == 3:
                score += 110
            ...
            # Partition bonus: Block opponent from unexplored areas
            ...
            if unexplored_blocked >= 8:
                score += 120  # Cutting off many tiles!
            elif unexplored_blocked >= 4:
                score += 60
        
        # 7. OPPONENT'S EGG PARITY
        if self.my_egg_parity is not None:
            opp_parity = 1 - self.my_egg_parity
            if (cx + cy) % 2 == opp_parity:
                score += 100
        
        # 8. DON'T BLOCK OURSELVES
    ...
    def _score_move(self, move, board_state, cx, cy) -> float:
        """
        APEX: Aggressive exploration with STRICT safety thresholds.
        
        Key: High exploration bonuses but STRICT risk requirements.
        """
        d, mt = move
        ...
        # APEX: STRICT thresholds for 0.05 traps/game (from Void)
        # But relax more in late game when board is known
        if very_late_game:
            RISK_VERY_SAFE = 0.08
            RISK_SAFE = 0.11
            ...
            # STRICT early/mid game - this is key for 0.05 traps/game
        ...
        late_bonus = 1.35 if very_late_game else (1.20 if late_game else 1.0)
        
        # APEX: HIGH exploration bonuses (from Dave) but require strict safety
        aggression_bonus = 1.25 if self.board_control < 0.5 else 1.0
        
        if is_confirmed_safe:
            if not is_visited:
                util += int(2200 * late_bonus * aggression_bonus)  # Very high!
            else:
                util += 18
                
        elif risk < RISK_VERY_SAFE and confidence >= CONFIDENCE_HIGH:
            if not is_visited:
                util += int(1700 * late_bonus * aggression_bonus)
            else:
                util += 14
                
        elif risk < RISK_SAFE and confidence >= CONFIDENCE_GOOD:
            if not is_visited:
                util += int(1300 * late_bonus * aggression_bonus)
            else:
                util += 10
                
        elif risk < RISK_ACCEPTABLE and confidence >= CONFIDENCE_OK and safe_neighbors >= 2:
            if not is_visited:
                util += int(900 * late_bonus)
            else:
                util += 8
                
        elif risk < RISK_CAUTION and in_safe_zone and evidence > 5:
            if not is_visited:
                util += int(600 * late_bonus)
            else:
                util += 5
        ...
        # Edge preference (early game safety)
        if self.turn_index <= 18:
