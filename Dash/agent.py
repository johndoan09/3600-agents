...
# ================================================================
# PlayerAgent – DASH (Black Chicken Strategy + Trap Memory)
# ================================================================
class PlayerAgent:
    """
    DASH: Black Chicken strategy with enhanced trap memory.
    ...
    5. TRAP MEMORY: Remember where we hit traps, mark adjacent as SAFE
    6. PATIENCE: Don't rush center until you have enough safety data
    """
    ...
    def __init__(self, board: board.Board, time_left: Callable):
        ...
        # Track our movement to detect trap hits
    ...
    def _record_trap(self, loc):
        """
        Remember trap location. Does NOT mark adjacent as safe.
        Instead, use this to be hyper aggressive in CONFIRMED safe zones.
        """
        x, y = loc
        if loc in self.confirmed_traps:
        ...
        # Remove from probable traps
        self.probable_traps.discard(loc)
        
        # DON'T mark adjacent as safe - there's still another trap out there!
        # But DO reduce danger signals slightly for tiles that were pointing here
        # (Now we know THIS tile is the trap, not nearby ones with signals)
        for dy in range(-2, 3):
            for dx in range(-2, 3):
                nx, ny = x + dx, y + dy
                if 0 <= nx < self.board_size and 0 <= ny < self.board_size:
                    if (nx, ny) not in self.confirmed_traps:
                        # Slightly reduce danger (some signals were for THIS trap)
                        # But don't clear completely - other trap still exists!
                        self.danger_signals[ny][nx] = max(0, self.danger_signals[ny][nx] - 1)
    ...
    def _score_move(self, move, board_state, cx, cy) -> float:
        ...
        # When we have a large safe zone AND found a trap, be more aggressive!
        safe_zone_size = len(self.silence_confirmed) + len(self.visited)
        traps_found = len(self.confirmed_traps)
        
        # Confidence level: high when we've explored a lot and found traps
        is_confident = safe_zone_size >= 20 or (traps_found >= 1 and safe_zone_size >= 12)
        
        # ===== SAFETY SCORING (with aggression boost) =====
        if is_safe and is_new:
            # BEST: New tile that's confirmed safe - HUGE bonus
            util += 1200 if is_confident else 1000
        elif is_safe:
            # Good: Revisiting safe tile
            util += 500
        elif is_new and danger == 0:
            # Unknown but no danger signals
            if is_confident:
                # HYPER AGGRESSIVE: Take unknown tiles when confident!
                util += 50  # Positive! Go explore!
            else:
                util -= 100  # Cautious early game
        elif is_new and danger <= 2:
            # Some suspicion
            if is_confident:
                util -= 200  # Less penalty when confident
            else:
                util -= 500
        elif is_new:
            # High suspicion - still avoid
            util -= 800
        else:
    ...
    def _choose_move(self, board_state, sensors, time_left):
        ...
        # PRIORITY 3: Low risk unknown moves
        if low_risk_moves:
            scored = [(self._score_move(m, board_state, cx, cy), m) for m in low_risk_moves]
            scored.sort(key=lambda x: x[0], reverse=True)
            
            # Check if we're in HYPER AGGRESSIVE mode
            safe_zone_size = len(self.silence_confirmed) + len(self.visited)
            traps_found = len(self.confirmed_traps)
            is_confident = safe_zone_size >= 20 or (traps_found >= 1 and safe_zone_size >= 12)
            
            if is_confident:
                # HYPER AGGRESSIVE: Take the best low-risk move without edge preference!
                return scored[0][1]
            else:
                # Cautious: prefer edges
                best_move = scored[0][1]
                nx, ny = self._apply_dir((cx, cy), best_move[0])
                if self._is_near_edge(nx, ny):
                    return best_move
                # Check for edge option
    ...
    def play(self, board_state: board.Board, sensors, time_left: Callable):
        ...
        # Record spawn position on first turn
        if self.spawn_position is None:
            self.spawn_position = (cx, cy)
        
        # ========== DETECT IF WE HIT A TRAP ==========
        # If we intended to move somewhere but ended up at spawn, we hit a trap!
        if self.intended_position is not None and self.last_position is not None:
            intended = self.intended_position
            current = (cx, cy)
            
            # If we're at spawn but intended to go somewhere else, we hit a trap
            if current == self.spawn_position and intended != self.spawn_position:
                # We hit a trap at our intended position!
                self._record_trap(intended)
            # Also check if we didn't move where we intended (trap detection)
            elif current != intended and current == self.last_position:
                # We tried to move but stayed in place - might have hit adjacent trap
                # The intended tile is likely the trap
                self._record_trap(intended)
        ...
        # Record found traps from game state
        try:
        ...
        # Track our movement for next turn's trap detection
