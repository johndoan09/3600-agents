...
# ================================================================
# PlayerAgent – ALPHA (Improved Trapdoor Detection)
# ================================================================
class PlayerAgent:
    """
    ALPHA: Based on Prime with IMPROVED trapdoor detection
    - Danger signal accumulation tracking
    - Stronger signal penalties
    - Better silence handling
    - More conservative on unconfirmed tiles
    """
    ...
    def __init__(self, board: board.Board, time_left: Callable):
        ...
        # DANGER SIGNAL ACCUMULATOR - tracks how many signals point to each tile
        self.danger_count = np.zeros((self.board_size, self.board_size))
        
        # SIGNAL HISTORY - track which positions we've already processed signals from
        # This prevents double-counting when revisiting tiles
        ...
        # Desperation mode - when stuck too long, take more risks
        ...
        # Track opponent's visited tiles (they're confirmed safe!)
    ...
    def _bayes_update(self, loc, sensors):
        ...
        # Check if we've already processed signals from this position
        first_time_here = loc not in self.signal_processed_from
        self.signal_processed_from.add(loc)

        # Always update Bayesian beliefs (proper probabilistic update)
        ...
        if hw or fw or hb or fb:
            ...
            for dy in range(-2, 3):
                for dx in range(-2, 3):
                    ...
                    if 0 <= nx < self.board_size and 0 <= ny < self.board_size:
                        # Skip tiles we've confirmed safe by visiting
                        if (nx, ny) in self.visited:
                            continue
                        
                        dist = abs(dx) + abs(dy)
                        if dist <= 2 and dist > 0:
                            # Only accumulate danger_count on FIRST observation from this position
                            # But always update safety_confidence (can get multiple readings)
                            if felt_signal and dist == 1:
                                # FELT = trapdoor ADJACENT! EXTREMELY dangerous!
                                ...
                                # Heard, adjacent - still very suspicious
            ...
            # SILENCE = No trapdoor within 2 tiles! Strong safety confirmation
            for dy in range(-2, 3):
                for dx in range(-2, 3):
                    nx, ny = lx + dx, ly + dy
                    if 0 <= nx < self.board_size and 0 <= ny < self.board_size:
    ...
    def _is_oscillating(self) -> bool:
        """Detect small-loop oscillation (2-4 tiles)"""
        if len(self.recent_positions) < 4:
            return False
        
        # Check last 4 positions for A-B-A-B pattern (2-tile loop)
        last4 = self.recent_positions[-4:]
        if last4[0] == last4[2] and last4[1] == last4[3] and last4[0] != last4[1]:
            return True
        
        # Check last 3 for A-B-A pattern
        if len(self.recent_positions) >= 3:
            last3 = self.recent_positions[-3:]
            if last3[0] == last3[2] and last3[0] != last3[1]:
                return True
        
        # Check for 3-4 tile loops: if last 6-8 positions only have 3-4 unique tiles
    ...
    def _get_oscillation_tiles(self) -> Set[Tuple[int, int]]:
        """Get all tiles in the oscillation loop (so we can avoid them)"""
        if len(self.recent_positions) < 4:
            return set()
        
        # Return all recently visited tiles that form the loop
        last6 = self.recent_positions[-6:] if len(self.recent_positions) >= 6 else self.recent_positions[-4:]
        return set(last6)
    
    def _get_oscillation_target(self) -> Optional[Tuple[int, int]]:
        """Get the tile we keep bouncing to (so we can avoid it)"""
        if len(self.recent_positions) < 2:
            return None
        # The tile we were just at (that we'd bounce back to)
        return self.recent_positions[-2] if len(self.recent_positions) >= 2 else None
    ...
    def _score_move(self, move, board_state, cx, cy, is_revisit: bool) -> float:
        """Score a move with IMPROVED trapdoor detection"""
        d, mt = move
        ...
        if is_revisit:
            ...
            # OSCILLATION DETECTION: heavily penalize ALL tiles in the oscillation loop
            if self._is_oscillating():
                oscillation_tiles = self._get_oscillation_tiles()
                if (nx, ny) in oscillation_tiles:
                    # This tile is part of our loop - MASSIVE penalty
                    return -1e9
        ...
        # Early game: be CAUTIOUS (don't know where traps are)
        # Late game: be CONFIDENT (we have data, know which tiles are safe)
        # + Desperation mode when stuck
        # + Competitive mode when falling behind
        
        # Knowledge factor: increases as we gather more sensor data
        tiles_explored = len(self.visited)
        knowledge_factor = min(1.0, tiles_explored / 25.0)  # 0 at start, 1.0 after 25 tiles
        
        # COMPETITIVE AWARENESS: be more aggressive when falling behind
        egg_deficit = self.enemy_eggs - self.my_eggs
        competitive_bonus = 0.0
        if egg_deficit >= 3:
            competitive_bonus = min(0.3, egg_deficit * 0.05)  # Up to 30% more aggressive
        
        # Early game: penalties are HIGHER (more cautious)
        # Late game: penalties are LOWER for tiles with GOOD confidence (we know they're safe)
        # But still high for tiles with BAD confidence (danger signals)
        
        # Base factor: start cautious, become confident as we explore
        confidence_from_knowledge = knowledge_factor * 0.3  # Up to 30% reduction from knowledge
        desperation_bonus = self.desperation_level * 0.07   # Up to 70% reduction from desperation
        
        # Only reduce penalties for tiles we have POSITIVE info about
        if confidence > 20:
            # We have good data about this tile - trust it
            desperation_factor = max(0.3, 1.0 - confidence_from_knowledge - desperation_bonus - competitive_bonus)
        else:
            # Unknown or dangerous tile - stay cautious, only desperation and competition help
            desperation_factor = max(0.4, 1.0 - desperation_bonus * 0.5 - competitive_bonus * 0.5)
        
        # High danger count = almost certainly a trapdoor
        if danger >= 5:
            util -= int(1200 * desperation_factor)  # Reduced when desperate
        elif danger >= 3:
        ...
        # Strong negative confidence = danger signal received
        if confidence < -50:
        ...
        # Confidence bonus (capped) - only if positive
        if confidence > 0:
            util += min(confidence * 2, 60)
        util -= risk * 120
        
        # BONUS: Tile opponent has visited = CONFIRMED SAFE
        if (nx, ny) in self.enemy_visited:
            util += 150  # Strong bonus for known-safe tiles
    ...
    def _choose_move(self, board_state, sensors, time_left):
        ...
        # Lower acceptance threshold when oscillating (we NEED to break out)
        acceptance_threshold = -300
        
        # Competitive pressure: lower threshold when behind
        egg_deficit = self.enemy_eggs - self.my_eggs
        if egg_deficit >= 5:
            acceptance_threshold = -600  # Way behind - take more risks
        elif egg_deficit >= 3:
            acceptance_threshold = -450  # Behind - take some risks
        
        if self._is_oscillating():
            acceptance_threshold = -1000  # MUST escape oscillation
        elif self._is_stuck_in_loop():
            acceptance_threshold = -700  # Need to escape loop
    ...
    def play(self, board_state: board.Board, sensors, time_left: Callable):
        ...
        try:
            ...
            # LEARN FROM OPPONENT: Track their movements
            if self.enemy_loc is not None:
                ex, ey = self.enemy_loc
                
                # If opponent is at a new location, that tile is CONFIRMED SAFE
                if (ex, ey) not in self.enemy_visited:
                    self.enemy_visited.add((ex, ey))
                    
                    # Mark as safe in our belief system
                    ...
                    # Also boost confidence in adjacent tiles (opponent survived there)
                    for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                        nx, ny = ex + dx, ey + dy
                        if 0 <= nx < self.board_size and 0 <= ny < self.board_size:
                            # Adjacent to a confirmed safe tile = less likely to be trap
        ...
        if (cx, cy) not in self.visited:
            self.turns_since_new_tile = 0
            self.desperation_level = max(0, self.desperation_level - 2)  # Reset desperation
        else:
            self.turns_since_new_tile += 1
            # Increase desperation when stuck
            if self.turns_since_new_tile >= 3:
                self.desperation_level = min(10, self.desperation_level + 1)
            # OSCILLATION = immediate high desperation
            if self._is_oscillating():
